...
homeworkData = pandas.read_csv('Homework_Customer.csv')
homeworkCustomer = homeworkData.iloc[:, 3:13].values
...
homeworkCustomer[:,1] = labelencoder_X_1.fit_transform(homeworkCustomer[:, 1])
homeworkCustomer[:,2] = labelencoder_X_2.fit_transform(homeworkCustomer[:, 2])
...
homeworkCustomer = onehotencoder.transform(homeworkCustomer).toarray()
...
homeworkCustomer = sc.transform(homeworkCustomer)
...
hw_pred = myNeuralNetwork.predict(homeworkCustomer)


# Predicting a single new observation (Based on the solution file)
# The double pair of brackets here specifies a 'horizontal' matrix, rather than a vertical one.
new_prediction = myNeuralNetwork.predict(sc.transform(np.array([[ 0, 0, 600, 1,  40, 3, 60000, 2, 1, 1, 50000]])))
